...
- **`orca status` now explains *why* a service is failing, K8s-style.** Failures used to be transient — returned to the CLI on the failing `orca deploy` and otherwise only in logs — so a service that failed to come up (or a running one that started crash-looping) showed as `stopped`/`degraded` with no reason. The control plane now records a per-service `last_failure` (reason + message + exit code + restart count): deploy-time errors are classified (`ImagePullError`, `AgentUnreachable`, `DeployTimeout`, `DeployError`) and carry the real underlying error, while runtime crashes are detected from agent heartbeats — the agent fetches each non-running container's exit code, restart count, and a `docker logs` tail and reports them, and the master classifies them (`CrashLoopBackOff`, `Error`). The reason is surfaced in `orca status` (an inline `↳ reason: …` line), the status API (`last_failure` field), and the TUI detail pane — but only while the service is actually degraded/stopped, and it's cleared once the service deploys or reports running again.
...
- **Large uploads through the proxy (e.g. Docker registry blob pushes >2 GB) no longer fail partway.** The proxy's upstream HTTP client used a *total* request timeout (300s), which capped every request by wall-clock — including the time spent streaming the request body. A large blob push over a typical link takes longer than 300s and was killed mid-upload (the symptom: "can't upload more than ~2 GB"). Replaced the total timeout with an **inactivity** (`read_timeout`) timeout: it still recovers from a hung/stalled backend (the original reason timeouts were added) but never caps a transfer that keeps making progress, so arbitrarily large blobs go through. Also stopped forwarding the client's `Content-Length` / `Transfer-Encoding` to the backend — reqwest sets body framing itself, and pairing a stale `Content-Length` with the chunked streamed body was a framing conflict that could truncate or stall large pushes. Regression test streams a multi-MiB body that arrives over wall-clock time and asserts it round-trips cleanly.
